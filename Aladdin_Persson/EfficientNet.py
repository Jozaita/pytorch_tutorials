...
phi_values = {
    #(phis_value,resolution,drop_rate)
    "b0":(0,224,0.2),
    "b1":(0.5,240,0.2),
    "b2":(1,260,0.3),
    "b3":(2,300,0.3),
    ...
    def create_features(self,width_factor,depth_factor,last_channels):
        ...
        for expand_ratio,channels,repeats,stride,kernel_size in base_model:
            ...
            for layer in range(layers_repeats):
                features.append(
                    InvertedResidualBlock(in_channels,out_channels,expand_ratio=expand_ratio,stride=stride if layer ==0 else 1,kernel_size=kernel_size,padding=kernel_size//2)
                )
